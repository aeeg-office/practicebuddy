    ...
    <div className="flex min-h-screen bg-[#f6f6f6]">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 shrink-0 bg-white border-r border-gray-200">
        ...
        <Link href="/parent" className="flex items-center gap-3 px-6 h-16 border-b border-gray-100">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-[rgb(11,79,74)] to-[rgb(22,32,34)] text-white text-sm font-bold">
            {sidebar.logo}
          </div>
          <div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-4 md:px-8 h-16 flex items-center justify-between">
          <div>
            <h1 className="text-lg md:text-xl font-bold text-[rgb(22,32,34)]">
              {dashboard.greeting} <span className="text-[rgb(11,79,74)]">{dashboard.userName}</span>
            </h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-2 border-l border-gray-200">
              <div className="hidden text-right sm:block">
                <p className="text-sm font-medium text-[rgb(22,32,34)] leading-tight">{topBar.userName}</p>
                <p className="text-[11px] text-gray-500">{topBar.userRole}</p>
              </div>
              <Avatar fallback="SP" size="md" className="ring-2 ring-[rgb(11,79,74)]/20" />
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-6">
          ...
          <section>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {(stats ?? []).map((stat: { label: string; value: string | number }, idx: number) => {
                ...
                const iconColors = [
                  "bg-gradient-to-br from-[rgb(11,79,74)]/10 to-[rgb(22,32,34)]/10 text-[rgb(11,79,74)]",
                  "bg-gradient-to-br from-emerald-500/10 to-emerald-600/10 text-emerald-600",
                  "bg-gradient-to-br from-[rgb(200,120,90)]/10 to-amber-600/10 text-[rgb(200,120,90)]",
                ...
                return (
                  <Card key={stat.label} className="border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-5">
                      ...
                      <p className="mt-3 text-2xl font-bold text-[rgb(22,32,34)]">{stat.value}</p>
              ...
              {(children ?? []).map((child: {
                ...
                <Card key={child.name} className="border border-gray-100 shadow-sm overflow-hidden">
                  <div className="md:flex">
                    ...
                    <div className="md:w-56 bg-gradient-to-br from-[rgb(11,79,74)] to-[rgb(22,32,34)] p-6 text-white flex flex-col items-center md:items-start justify-start">
                    ...
                    <div className="flex-1 p-5">
                      {/* Recent Scores */}
                      <div className="mb-4">
                        <h4 className="text-sm font-semibold text-[rgb(22,32,34)] mb-3 flex items-center gap-1.5">
                          <Award className="h-4 w-4 text-[rgb(200,120,90)]" />
                          Recent Scores
                        </h4>
                            ...
                            return (
                              <div key={score.subject} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                ...
                                <div className="flex items-baseline gap-1 mt-1">
                                  <span className="text-xl font-bold text-[rgb(22,32,34)]">{score.score}</span>
                          ...
                          {(child.upcomingSessions ?? []).map((session: { date: string; time: string; subject: string; teacher: string; type: string }, i: number) => (
                            ...
                            >
                              <div className="flex flex-col items-center w-16 shrink-0">
                                <span className="text-[10px] font-semibold text-[rgb(11,79,74)] uppercase leading-tight">
                                  {session.date.split(",")[0]}
                                </span>
                                <span className="text-[9px] text-gray-500">{session.time}</span>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-[rgb(22,32,34)] truncate">{session.subject}</p>
                      ...
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="rounded-lg bg-[rgb(11,79,74)]/[0.04] border border-[rgb(11,79,74)]/10 p-3">
                          <div className="flex items-center gap-2 mb-1.5">
                            <MessageSquare className="h-3.5 w-3.5 text-[rgb(11,79,74)]" />
                            <span className="text-[11px] font-semibold text-[rgb(22,32,34)]">Teacher Comment</span>
                          </div>
                          <p className="text-xs text-gray-600 leading-relaxed">{child.teacherComment}</p>
                        </div>
                        <div className="rounded-lg bg-amber-50 border border-amber-100 p-3">
                          <div className="flex items-center gap-2 mb-1.5">
                            <Star className="h-3.5 w-3.5 text-[rgb(200,120,90)]" />
                            <span className="text-[11px] font-semibold text-[rgb(22,32,34)]">Recommendation</span>
                          </div>
            ...
            <div className="space-y-6">
              ...
              <Card className="border border-gray-100 shadow-sm">
                <CardHeader className="pb-3 flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-sm font-semibold text-[rgb(22,32,34)]">{paymentSummary.title}</CardTitle>
                    ...
                    {(paymentSummary.recent ?? []).map((payment: { date: string; description: string; amount: number; status: string }, i: number) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-xs font-medium text-[rgb(22,32,34)] truncate max-w-[140px]">{payment.description}</p>
                            <p className="text-[10px] text-gray-400">{payment.date}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-[rgb(22,32,34)]">${payment.amount}</span>
                  ...
                  {paymentSummary.nextPayment?.amount && (
                    <div className="mt-3 rounded-lg bg-amber-50 border border-amber-200 p-3 flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-[rgb(22,32,34)]">Next Payment</p>
                          <p className="text-[10px] text-gray-500">Due {paymentSummary.nextPayment.dueDate}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-[rgb(22,32,34)]">${paymentSummary.nextPayment.amount}</p>
                        <Link href="/parent/payments">
              ...
              <Card className="border border-gray-100 shadow-sm">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold text-[rgb(22,32,34)] flex items-center gap-2">
                    <MessageSquare className="h-4 w-4 text-[rgb(11,79,74)]" />
                      ...
                      <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-100">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-semibold text-[rgb(22,32,34)]">{item.teacher}</span>
                            <span className="text-[10px] text-gray-400">{item.date}</span>
                          </div>
                          <p className="text-xs text-gray-600 mt-0.5 leading-relaxed">
                            <span className="font-medium text-[rgb(11,79,74)]">@{item.child}</span> — {item.comment}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              {/* Tip Card */}
              <Card className="border-0 bg-gradient-to-br from-[rgb(11,79,74)] to-[rgb(22,32,34)] shadow-md">
                <CardContent className="p-5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10">
                      <Sparkles className="h-5 w-5 text-[rgb(200,120,90)]" />
                    </div>
                    <div>
              ...
              >
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-[rgb(22,32,34)]">WhatsApp Support</h4>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(overallStats ?? []).map((stat: { label: string; value: string | number }, idx: number) => {
              ...
              const iconColors = [
                "bg-[rgb(11,79,74)]/10 text-[rgb(11,79,74)]",
                "bg-[rgb(22,32,34)]/10 text-[rgb(22,32,34)]",
                "bg-emerald-500/10 text-emerald-600",
                "bg-[rgb(200,120,90)]/10 text-[rgb(200,120,90)]",
              ]
                ...
                <Card key={stat.label} className="border border-gray-100 shadow-sm">
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-[rgb(22,32,34)]">{stat.value}</p>
                    </div>
                  </CardContent>
                </Card>
              )
