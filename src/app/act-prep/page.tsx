...
const whyIcons = [Award, Target, Users, BookOpen, GraduationCap, MessageSquare] as const
const programIcons: Record<string, typeof Globe> = {
  "online-group": Globe,
  "in-center": Building2,
  "online-private": Monitor,
}

export default function ActPrepPage() {
  const [openFaq, setOpenFaq] = useState<number | null>(null)
  const c = usePageContent("act-prep")

  const toggleFaq = (index: number) => {
    setOpenFaq(openFaq === index ? null : index)
  }

  const whyItems = (c.whyAEEG?.items ?? []).map((item: { title: string; description: string }, i: number) => ({
    ...item,
    icon: whyIcons[i] ?? Award,
  }))

  const programItems = (c.programs?.items ?? []).map(
    (p: { name: string; price: string; period: string; description: string; features: string[]; popular: boolean; cta: string }) => ({
      ...p,
      icon: programIcons[p.name === "Online Tutoring" ? "online-group" : p.name === "In-Center Private Tutoring" ? "in-center" : "online-private"] ?? Globe,
    })
  )
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="bg-background py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {whyItems.map((item: { icon: React.ComponentType<{ className?: string }>; title: string; description: string }) => {
      ...
      <section id="programs" className="bg-muted/30 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-16 grid grid-cols-1 gap-8 lg:grid-cols-3">
            {programItems.map((program: { name: string; price: string; period: string; description: string; features: string[]; popular: boolean; cta: string; icon: React.ComponentType<{ className?: string }> }) => {
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-12 overflow-hidden rounded-2xl border border-border/60 shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody>
                  {(c.testDates?.dates ?? []).map((row: { date: string; registration: string; late: string }, index: number) => (
                    <tr
                      key={row.date}
          ...
          <div className="mt-8 rounded-xl bg-muted/30 p-6 text-center">
            <p className="text-sm text-muted-foreground">
              <span className="font-semibold text-[#1e2761]">Note:</span> {c.testDates?.note}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-28 lg:px-8">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              ...
              <a
                href="https://wa.me/201060618899?text=Hi%20AEEG!%20I%20want%20to%20ask%20about%20ACT%20preparation%20programs."
                target="_blank"
      ...
      <a
        ...
        aria-label={`Ask about ${c.cta?.floatingBadge ?? "ACT Prep"} on WhatsApp`}
      >
        <MessageCircle className="h-5 w-5" />
